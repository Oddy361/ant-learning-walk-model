...
"""
cart2sph 函数
    将笛卡尔坐标转换为球面坐标，计算方位角（TH）、仰角（PHI）和半径（R）。
pi2pi 函数
    将角度值约束在-π到π之间，确保角度值在合理范围内。
img_wrapper 函数
    将输入图像按照指定比例包裹成圆盘状，以适应后续处理需求。
get_img_view 函数
    从模拟世界中重建图像视图，包含以下步骤：
    将笛卡尔坐标转换为球面坐标。
    计算水平和垂直视场。
    使用Matplotlib绘制3D视图。
    将视图转换为图像，并进行灰度和模糊处理（如果需要）。
visual_sense 函数
    获取给定位置和视角的频率编码信息：
    调用get_img_view函数获取图像。
    计算图像的Zernike矩，提取其幅值和相位信息。
    这些函数组合在一起，为模拟昆虫大脑的视觉处理和归巢行为提供了基础图像处理和特征提取的功能
"""
def cart2sph(X, Y, Z):
